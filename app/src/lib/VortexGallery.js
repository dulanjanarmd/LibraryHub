...
void main() {
  ...
  vec4 color = texture2D(uAtl, atlasUV);
  gl_FragColor = color;
}
`;
...
export default class VortexGallery {
  ...
  async loadTextureAtlas(paths) {
    ...
    // Resize each image to fit within the atl
  ...
  /**
   * Hit-test a canv
   * image the user clicked on, or null if nothing w
   ...
   * small pixel radius.
   */
  pickAtScreen(clientX,  clientY,  canvasRect){
